...
const Camera= require('../Model/UserModel');// Importing the Camera model for MongoDB
const validateTokenMiddleware = require("../middlewares/validateTokenMiddleware");// Importing the middleware to validate user token
const axios= require('axios') // Importing axios to make HTTP requests


router.post("/signin", async (req, res) => {
    const { email_id, password } = req.body; // Destructuring email and password from request body
...
// Route to fetch camera feed for a specific camera ID, protected by token validation middleware
